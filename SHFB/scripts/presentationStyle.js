...
// In the createThemeToggle function, change the append location:
function createThemeToggle() {
    ...
    toggleButton.innerHTML = `
        <span class="theme-icon-light" style="display: none;">☀️ Light</span>
        <span class="theme-icon-dark" style="display: none;">🌙 Dark</span>
    `;

    // Insert into header instead of body
    $(document).ready(function () {
        const langContainer = document.getElementById('LangFilterSearchContainer');
        if (langContainer) {
            // Find the first level-item (which contains the dropdown)
            const firstLevelItem = langContainer.querySelector('.level-item');
            ...
            if (firstLevelItem) {
                ...
                // Insert before the language dropdown's level-item
                langContainer.insertBefore(levelItem, firstLevelItem);
            } else {
                // Fallback: just append to the container
                langContainer.appendChild(toggleButton);
            }
        } else {
...
function updateToggleButton(currentTheme) {
    ...
    if (currentTheme === 'dark') {
        // Show "Switch to Light" option
        lightIcon.style.display = 'none';
        darkIcon.style.display = 'inline';
    } else {
        // Show "Switch to Dark" option
...
window.toggleDocumentationTheme = toggleTheme;