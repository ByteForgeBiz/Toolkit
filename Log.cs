using System;
using System.IO;
using System.Linq;
using System.Runtime.CompilerServices;
using ByteForge.Toolkit.Logging;

namespace ByteForge.Toolkit;

/*
 *  _              
 * | |   ___  __ _ 
 * | |__/ _ \/ _` |
 * |____\___/\__, |
 *           |___/ 
 */
/// <summary>
/// Provides logging functionality for the application.
/// </summary>
public class Log : CompositeLogger
{
    private LogLevel _PreviousConsoleLoggerLevel = LogLevel.Verbose;
    private readonly ILogger _ConsoleLogger = new ConsoleLogger() { Name = "Console", MinLogLevel = LogLevel.Verbose };

    /// <summary>
    /// Initializes a new instance of the <see cref="Log"/> class.
    /// </summary>
    private Log() : base("TelecomInc.Common->Static Logger")
    {
        var logLevel = Configuration.TraceLogLevel;
        AddLogger(_ConsoleLogger);
        AddLogger(new FileLogger(Configuration.LogFilePath) { Name = "File", MinLogLevel = logLevel });
    }

    /// <summary>
    /// Gets a value indicating whether console logging is enabled.
    /// </summary>
    /// <value>
    /// <c>true</c> if console logging is enabled; otherwise, <c>false</c>.
    /// </value>
    public bool IsConsoleLoggingEnabled
    {
        get => _ConsoleLogger.MinLogLevel != LogLevel.None;
    }

    /// <summary>
    /// Gets the alternate logger instance, which logs to a temporary file.
    /// </summary>
    public ILogger AlternateLogger =>
                            _alternateLogger ??= new FileLogger(Path.Combine(Path.GetTempPath(), $"LogError.{Guid.NewGuid()}.log"))
                            {
                                Name = "Alternate",
                                MinLogLevel = LogLevel.Verbose
                            };
    private ILogger _alternateLogger;

    /// <summary>
    /// Gets the singleton instance of the Log class.
    /// </summary>
    public static Log Instance => _instance ??= new();
    private static Log _instance;

    /// <summary>
    /// Enables console logging.
    /// </summary>
    public static void EnableConsoleLogging() => Instance._ConsoleLogger.MinLogLevel = Instance._PreviousConsoleLoggerLevel;

    /// <summary>
    /// Disables console logging.
    /// </summary>
    public static void DisableConsoleLogging()
    {
        Instance._PreviousConsoleLoggerLevel = Instance._ConsoleLogger.MinLogLevel;
        Instance._ConsoleLogger.MinLogLevel = LogLevel.None;
    }

    /// <summary>
    /// Gets or sets the log level for console logging.
    /// </summary>
    public static LogLevel ConsoleLogLevel
    {
        get => Instance._ConsoleLogger.MinLogLevel;
        set => Instance._ConsoleLogger.MinLogLevel = value;
    }

    /// <summary>
    /// Gets or sets the log level for all loggers.
    /// </summary>
    public static LogLevel LogLevel
    {
        get => Instance.Loggers.Where(l => l.MinLogLevel != LogLevel.None).Max(l => l.MinLogLevel);
        set
        {
            Instance.LogVerbose($"Setting log level to {value}.");
            Instance.Loggers.ForEach(l => l.MinLogLevel = value);
        }
    }

    /// <summary>
    /// Logs a verbose message.
    /// </summary>
    /// <param name="message">The message to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Verbose(string message, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Verbose, message, null, source);

    /// <summary>
    /// Logs a debug message.
    /// </summary>
    /// <param name="message">The message to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Debug(string message, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Debug, message, null, source);

    /// <summary>
    /// Logs an informational message.
    /// </summary>
    /// <param name="message">The message to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Info(string message, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Info, message, null, source);

    /// <summary>
    /// Logs a warning message.
    /// </summary>
    /// <param name="message">The message to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Warning(string message, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Warning, message, null, source);

    /// <summary>
    /// Logs a warning message.
    /// </summary>
    /// <param name="message">The message to log.</param>
    /// <param name="ex">The exception to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Warning(string message, Exception ex, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Warning, message, ex, source);

    /// <summary>
    /// Logs an error message.
    /// </summary>
    /// <param name="message">The message to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Error(string message, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Error, message, null, source);

    /// <summary>
    /// Logs an error message with an exception.
    /// </summary>
    /// <param name="ex">The exception to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Error(Exception ex, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Error, ex.Message, ex, source);

    /// <summary>
    /// Logs an error message with an exception and a custom message.
    /// </summary>
    /// <param name="message">The message to log.</param>
    /// <param name="ex">The exception to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Error(string message, Exception ex, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Error, message, ex, source);

    /// <summary>
    /// Logs a critical error message.
    /// </summary>
    /// <param name="message">The message to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Critical(string message, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Critical, message, null, source);

    /// <summary>
    /// Logs a critical error message with an exception.
    /// </summary>
    /// <param name="ex">The exception to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Critical(Exception ex, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Critical, ex.Message, ex, source);

    /// <summary>
    /// Logs a critical error message with an exception and a custom message.
    /// </summary>
    /// <param name="message">The message to log.</param>
    /// <param name="ex">The exception to log.</param>
    /// <param name="source">The source of the message. This is automatically set to the caller member name.</param>
    public static void Critical(string message, Exception ex, [CallerMemberName] string source = "") => WriteToLog(LogLevel.Critical, message, ex, source);

    /// <summary>
    /// Writes a message to a specified log file.
    /// </summary>
    /// <param name="level">The trace level of the message.</param>
    /// <param name="message">The message to write.</param>
    /// <param name="ex"></param>
    /// <param name="source">The source of the message.</param>
    /// 
    private static void WriteToLog(LogLevel level, string message, Exception ex, string source)
    {
        if (string.IsNullOrEmpty(message))
            return;
        Instance.WriteToLog(level, message.Split(Utils.arrCRLF, StringSplitOptions.RemoveEmptyEntries), ex, source);
    }

    /// <summary>
    /// Writes a set of messages to a specified log file.
    /// </summary>
    /// <param name="level">The trace level of the messages.</param>
    /// <param name="text">The messages to write.</param>
    /// <param name="ex"></param>
    /// <param name="source">The source of the messages.</param>
    /// 
    private void WriteToLog(LogLevel level, string[] text, Exception ex, string source)
    {
        if (text == null || 
            text.Length == 0 || 
            (text.Length == 1 && string.IsNullOrEmpty(text[0])) ||
            level < Configuration.TraceLogLevel)
            return;

        try
        {
            Instance.Log(level, string.Join("\n", text), source, ex);
        }
        catch (Exception ex2)
        {
            /*
             * Generally, if an error occurs in this method, it is due to a permissions issue.
             * So, we will attempt to write the error to a temporary file in the user's temp directory,
             * as the user usually have write permissions there.
             */
            AlternateLogger.LogCritical("An error occurred while writing to the log file.", "Log.WriteToFile", ex2);
            AlternateLogger.Log(level, string.Join("\n", text), source, ex);
        }
    }
}
