...
namespace ByteForge.Toolkit
{
    public static partial class Core
    {
        ...
        private class WinScpResourceManager
        {
            ...
            private const string WINSCPNET_DLL = "WinSCPnet.dll";
            private const string WINSCP_EXE_RESOURCE = TOOLKIT + ".Dependencies." + WINSCP_EXE;
            private const string WINSCPNET_DLL_RESOURCE = TOOLKIT + ".Dependencies." + WINSCPNET_DLL;
            ...
            public void EnsureWinScpFilesAvailable()
            {
                ...
                EnsureFileAvailable(WINSCPNET_DLL_RESOURCE, WINSCPNET_DLL);
            }
            ...
            private bool IsFileUpToDate(string resourceName, string filePath)
            {
                try
                {
                    byte[] resourceChecksum = CalculateResourceChecksum(resourceName);
                    byte[] fileChecksum = CalculateFileChecksum(filePath);
            ...
            /// <returns>Byte array containing the checksum.</returns>
            private byte[] CalculateFileChecksum(string filePath)
            {
            ...
            private byte[] CalculateChecksum(Stream stream)
            {
            ...
            public bool AreWinScpFilesAvailable()
            {
                ...
                var winScpNetPath = Path.Combine(_executionPath, WINSCPNET_DLL);

                return File.Exists(winScpExePath) && File.Exists(winScpNetPath) &&
                       IsFileUpToDate(WINSCP_EXE_RESOURCE, winScpExePath) &&
                       IsFileUpToDate(WINSCPNET_DLL_RESOURCE, winScpNetPath);
            }
            ...
            public void CleanupWinScpFiles()
            {
                ...
                try
                {
                    ...
                    if (File.Exists(winScpNetPath))
                        File.Delete(winScpNetPath);
                }
                catch
                {
